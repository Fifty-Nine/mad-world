...
class NoZeroBidsEvent(BaseEventCard):
    card_kind: ClassVar[str] = "event_no_zero_bids"

    title: str = "Tense Negotiations"
    description: str = (
        "Diplomatic channels are frozen. Players cannot bid 0 this round."
    )

    def run(self, game: GameState) -> list[GameEvent]:
        # Expire at the end of the current round.
        game.active_effects.append(
            NoZeroBidsEffect(expiration_round=game.current_round)
        )
        return [
            GameEvent(
                actor=SystemActor(),
                description=f"Event: {self.title} - {self.description}",
            )
        ]


class NoDomesticInvestmentEvent(BaseEventCard):
    card_kind: ClassVar[str] = "event_no_domestic_investment"

    title: str = "Global Distraction"
    description: str = (
        "A major international incident dominates headlines. Domestic "
        "investment is disallowed for this round and the next."
    )

    def run(self, game: GameState) -> list[GameEvent]:
        # Expire at the end of the next round.
        game.active_effects.append(
            NoDomesticInvestmentEffect(expiration_round=game.current_round + 1)
        )
        return [
            GameEvent(
                actor=SystemActor(),
