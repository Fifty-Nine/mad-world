...
from mad_world.effects import NoZeroBidsEffect
...
def test_no_zero_bids_event(basic_game: GameState) -> None:
    event = NoZeroBidsEvent()
    basic_game.current_round = 5
    game_events = event.run(basic_game)

    assert len(game_events) == 1
    assert len(basic_game.active_effects) == 1
    assert basic_game.active_effects[0].card_kind == "effect_no_zero_bids"
    assert basic_game.active_effects[0].expiration_round == 5


def test_no_domestic_investment_event(basic_game: GameState) -> None:
    event = NoDomesticInvestmentEvent()
    basic_game.current_round = 5
    game_events = event.run(basic_game)

    assert len(game_events) == 1
    assert len(basic_game.active_effects) == 1
    assert basic_game.active_effects[0].card_kind == (
        "effect_no_domestic_investment"
    )
    assert basic_game.active_effects[0].expiration_round == 6


@pytest.mark.asyncio
async def test_resolve_round_events_expiration(basic_game: GameState) -> None:
    basic_game.current_phase = GamePhase.ROUND_EVENTS
    basic_game.current_round = 2

    # Add an effect that expires after round 1
    effect = NoZeroBidsEffect(expiration_round=1)
    basic_game.active_effects.append(effect)

    new_game = await resolve_round_events(basic_game)

    # The effect should have expired
    assert len(new_game.active_effects) == 0

    # And a GameEvent should have been generated logging the expiration
    assert any("has expired" in e.description for e in new_game.event_log)
