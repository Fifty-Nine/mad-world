...
def test_resolve_ai_arms_race_tied_bids(basic_game: GameState, crisis: AIArmsRaceCrisis) -> None:
    actions = {
    ...
    assert results[2].description == "Both superpowers achieved parity in AI research, with neither gaining a definitive advantage."
    assert getattr(results[2], "world_ending", False) is False
    assert results[2].influence_delta == {}


def test_resolve_ai_arms_race_winner(basic_game: GameState, crisis: AIArmsRaceCrisis) -> None:
    actions = {
    ...
    assert results[1].gdp_delta == {"Omega": -10}
    assert results[2].description == f"Alpha secured AI dominance."
    assert results[2].influence_delta == {"Alpha": AIArmsRaceDefs.WINNER_INF}
    assert getattr(results[2], "world_ending", False) is False


def test_resolve_ai_arms_race_rogue_ai(basic_game: GameState, crisis: AIArmsRaceCrisis) -> None:
    actions = {
...
def test_default_action(basic_game: GameState, crisis: AIArmsRaceCrisis) -> None:
    agg_action = crisis.get_default_action("Alpha", basic_game, aggressive=True)
    dip_action = crisis.get_default_action("Alpha", basic_game, aggressive=False)
